...
void insercaoPorCor(int cor, int h){
    for (int i = (h + cor); i < n; i+=h) {
        ...
        int j = i - h;
        int k = -1; comp++;
        while ((j >= 0) && ((personagens[j]->peso) > (tmp -> peso))) {
            personagens[j + h] = personagens[j];
            j-=h; k++;
        }
        if (k!=-1) comp += k;

        personagens[j + h] = tmp; mov++;
    }
}
...
void shellsort() {
    ...
    for (int i = 0; i < n; i++){
        comp++;
        if (i < n-1 && (personagens[i] -> peso == personagens[i+1] -> peso)){

            inicio = i; fim = i;int j = i; int k = -1; comp++;

            while (personagens[j] -> peso == personagens[j+1] -> peso){
                fim++; k++; j++;
            }
