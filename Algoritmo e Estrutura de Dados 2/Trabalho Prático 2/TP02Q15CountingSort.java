...
class Lista {
    ...
    /**
	* Retorna o personagem com maior peso da lista.
    * @return personagem com maior peso.	 
    */
	public int MaiorPeso() {
        int maior = (int)array[0].getPeso(); mov++;
 
        for (int i = 0; i < n; i++) {
            comp++;
            if(maior < (int)array[i].getPeso()){
             maior = (int)array[i].getPeso();mov++;
          }
	...
	public String countingsort() {
        ...
        //Ordenando
        for(int i = n-1; i >= 0; count[(int)array[i].getPeso()]--, i--){
            
            mov++;
            ordenado[count[(int)array[i].getPeso()] - 1] = array[i].Clone(); 
        }
        
        //Copiando para o array original
        for(int i = 0; i < n; array[i] = ordenado[i].Clone(), i++,mov++);
...
class TP02Q15CountingSort {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileWriter arq = new FileWriter("matricula_countingsort.txt");
        PrintWriter gravarArq = new PrintWriter(arq);
        
        String compmov = personagens.countingsort();
