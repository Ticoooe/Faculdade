...
void quicksortRec (int esq, int dir) {
    ...
    while (i <= j) {
        ...
        while (strcmp(personagens[i] -> corDoCabelo, pivo -> corDoCabelo) < 0){ 
        i++; 
        } 
        
        while (strcmp(personagens[j] -> corDoCabelo,  pivo -> corDoCabelo) > 0){ 
        j--;
        } 
...
void quicksort() {
   ...
   quicksortRec(0, n-1);
    ...
    for (int i = 0; i < n; i++){
        ...
        if (i < n-1 && strcmp(personagens[i] -> corDoCabelo, personagens[i+1] -> corDoCabelo) == 0){

            inicio = i; fim = i;int j = i; int k = -1; comp++;

            while ( j + 1 < n && strcmp(personagens[j] -> corDoCabelo,personagens[j+1] -> corDoCabelo) == 0){
                fim++; k++;j++;
            }
