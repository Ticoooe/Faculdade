...
/**
	* Retorna a maior altura dos personagens.
    * @return int altura maior personagem.	 
    */
int MaiorAltura (){
    int max = personagens[0] -> altura; mov++;

    for (int i = 1; i < n; i++){
        comp++;
        if (personagens[i] -> altura > max){
            max = personagens[i] -> altura;mov++;
        }
    }
...
void countingSort(int exp){
    ...
    Personagem *output [100];
    int i, count [10] = {0};

    for (i = 0; i < n; i++){
        count[ (personagens[i] -> altura/exp) % 10 ]++; 
    }

    for (i = 1; i < 10; i++){
        count [i] += count [i - 1]; mov++;
    }

    for (i = n - 1; i >= 0; i--){
        output [count [ (personagens[i] -> altura/exp)%10 ] - 1] = personagens[i]; mov++;
        count[ (personagens[i] -> altura/exp)%10 ]--;
    }

    for (i = 0; i < n; i++){
        personagens[i] = output[i]; mov++;
    }
}
...
int main(){
    ...
    FILE *arq;
    arq = fopen("matricula_radixsort.txt","w");
