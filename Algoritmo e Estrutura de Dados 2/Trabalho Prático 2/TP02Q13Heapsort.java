    ...
    */
    ...
    public void reconstroi(int tamHeap){
      ...
      public String heapsort() {
        ...
        for (int i = 0; i <= n; i++){
            comp++;
            if (i < n - 1 && array[i].getAltura() == array[i+1].getAltura()){
                inicio = i; fim = i; int j = i;
                comp++; int k = -1;
                while (array[j].getAltura() == array[j+1].getAltura()){
                    fim++; j++; k++;
                }
                if (k!=-1){
                    comp += k;
...
class TP02Q13Heapsort {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileWriter arq = new FileWriter("matricula_heapsort.txt");
        PrintWriter gravarArq = new PrintWriter(arq);
        
        String compmov = personagens.heapsort();
